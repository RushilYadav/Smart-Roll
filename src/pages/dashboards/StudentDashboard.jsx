...
function StudentDashboard() {
    ...
    const sections = [
        ...
        {
            title: 'My Attendance',
            description: 'View attendance records and insights.',
            icon: '📅',
    ...
    return (
        <div className='min-h-screen bg-gray-100 p-6 relative'>
            <div className='absolute top-6 right-6'>
                <button onClick={() => setShowConfirmLogout(true)} className='text-red-600 font-medium hover:underline text-xl mr-3 mt-2'>Log Out</button>
            </div>
            {showConfirmLogout && (
                <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50'>
                    <div className='bg-white rounded-xl p-6 shadow-lg w-full max-w-sm text-centre'>
                        <h2 className='text-2xl font-semibold mb-4'>Log Out</h2>
                        <p className='mb-6'>Are you sure you want to log out?</p>
                        <div className='flex justify-center gap-4'>
                            <button onClick={handleLogout} className='bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700'>Yes</button>
                            <button onClick={() => setShowConfirmLogout(false)} className='bg-gray-300 px-4 py-2 rounded hover:bg-gray-400'>No</button>
                        </div>
                    </div>
                </div>
            )}

            <h1 className='text-4xl font-bold text-centre mb-8 mt-2 ml-3'>Student Dashboard</h1>
